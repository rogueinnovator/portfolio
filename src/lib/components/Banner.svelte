<script lang="ts">
  ...
  onMount(() => {
    ...
    .to(heroText, {
      ...
      duration: 1.2,
      ease: "power4.in",
      onComplete: () =>{ showBanner = false}
    });

    const glitchTl = gsap.timeline({ repeat: -1, repeatDelay: 2 });
    glitchTl.to(glitchOverlay, {
      opacity: 0.1,
      duration: 0.1
    })
    .to(glitchOverlay, {
...
>
  <div class="absolute inset-0">
    ...
    <div class="absolute inset-0 bg-gradient-to-br from-gray-900 via-black to-violet-950/30"></div>
    
    <!-- Glow effects matching violet color scheme -->
    <div class="absolute top-1/3 left-1/5 w-96 h-96 bg-violet-900/20 rounded-full mix-blend-multiply filter blur-3xl"></div>
    <div class="absolute bottom-1/3 right-1/5 w-96 h-96 bg-gray-800/30 rounded-full mix-blend-multiply filter blur-3xl"></div>
  </div>
  ...
  <div class="absolute inset-0 opacity-5">
    <div class="h-full w-full bg-gradient-to-b from-transparent via-violet-500/10 to-transparent bg-repeat-y animate-scan"></div>
  </div>
  ...
  <div class="relative z-10 text-center px-4">
    ...
    >
      <span class="bg-gradient-to-r from-gray-100 via-violet-200 to-gray-300 bg-clip-text text-transparent font-extrabold">
        Hi, I am
      </span>
      <br>
      <span class="bg-gradient-to-r from-violet-400 via-violet-500 to-violet-600 bg-clip-text text-transparent font-black tracking-wider relative">
        Huzaifa
        <!-- Subtle glow effect behind name -->
        <span class="absolute inset-0 bg-gradient-to-r from-violet-400 via-violet-500 to-violet-600 bg-clip-text text-transparent blur-sm opacity-50 -z-10">
          Huzaifa
        </span>
      </span>
    </h1>
    ...
    <div class="mt-8 mx-auto w-32 h-0.5 bg-gradient-to-r from-transparent via-violet-500 to-transparent shadow-lg shadow-violet-500/50"></div>
    
    <!-- Additional decorative elements -->
    <div class="absolute -top-4 -right-4 w-2 h-2 bg-violet-500 rounded-full animate-pulse"></div>
    <div class="absolute -bottom-4 -left-4 w-1 h-1 bg-violet-400 rounded-full animate-pulse delay-500"></div>
  </div>
